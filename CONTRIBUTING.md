# Contributing

Thanks for helping improve Meeting Note.

1. Open an issue for substantial behavior or architecture changes.
2. Keep changes focused and include tests for storage or formatting logic.
3. Run the app and verify microphone permission, start/stop, and Markdown output.
4. Never add analytics, remote transcription, or audio persistence without explicit opt-in UX.

By contributing, you agree that your contribution is licensed under the MIT License.
