...
APP_PATH="${DIST_DIR}/Local Transcribe.app"
CACHE_DIR="${PROJECT_ROOT}/.cache"
SCRATCH_DIR="${PROJECT_ROOT}/.build"
TEMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/local-transcribe-package.XXXXXX")"
...
if [[ "$(uname -m)" != "arm64" ]]; then
  print -u2 "Local Transcribe requires an Apple silicon Mac."
  exit 1
fi
...
ditto "${BIN_PATH}/MeetingNote" "${APP_PATH}/Contents/MacOS/LocalTranscribe"
...
ditto -c -k --sequesterRsrc --keepParent "${APP_PATH}" "${DIST_DIR}/LocalTranscribe-macOS-arm64.zip"

DMG_STAGE="${TEMP_DIR}/dmg"
mkdir -p "${DMG_STAGE}"
ditto "${APP_PATH}" "${DMG_STAGE}/Local Transcribe.app"
ln -s /Applications "${DMG_STAGE}/Applications"
hdiutil create -volname "Local Transcribe" -srcfolder "${DMG_STAGE}" -ov -format UDZO "${DIST_DIR}/LocalTranscribe-macOS-arm64.dmg" >/dev/null

print "Built ${APP_PATH}"
print "Packaged ${DIST_DIR}/LocalTranscribe-macOS-arm64.zip"
print "Packaged ${DIST_DIR}/LocalTranscribe-macOS-arm64.dmg"
